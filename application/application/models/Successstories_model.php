...
class Successstories_model extends CI_Model {
	...
	public function get_rows($id=null)
	{
		if(!empty($id))
		{
			$sql	=	$this->db->get_where($this->prefix.'success_stories', array('id'=>$id));
			return $sql->row();
		}
		else
		{
	...
	public function save_row($post)
	{
		...
		if(isset($_FILES))
		{
	        ...
	        if(!empty($post['id']))
			{
				...
				$this->db->update($this->prefix.'success_stories', $values);
				return $post['id'];
				
			}
			else
			{
			  $this->db->insert($this->prefix.'success_stories', $values);
